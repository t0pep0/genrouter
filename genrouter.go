...
func main() {
	...
	fmt.Fprintf(gen, "\t\"github.com/valyala/fasthttp\"\n")
	for pkg := range imports {
		fmt.Fprintf(gen, "\t\"%v\"\n", pkg)
	}
	fmt.Fprintf(gen, ")\n\n")
	fmt.Fprintf(gen, "const (\n")
	for index, rout := range routes {
		fmt.Fprintf(gen, "\troute%vID%v = \"%v%v\"\n", rout.Func, index, rout.Method, rout.Path)
	}
	fmt.Fprintf(gen, ")\n\n")
	fmt.Fprint(gen, "func Router(ctx *fasthttp.RequestCtx) {\n")
	fmt.Fprintf(gen, "\tswitch string(ctx.Method())+string(ctx.Path()) {\n")
	for index, rout := range routes {
		fmt.Fprintf(gen, "\t\tcase route%[1]vID%[2]v: %[3]v.%[1]v(ctx)\n", rout.Func, index, rout.Pkg)
	}
	fmt.Fprintf(gen, "\t}\n}\n")
}
